...
COMMAND_PASTE = \
    str('You can paste it as text into the properties of a new launcher.')
ERROR_INSTALL_ACTION = \
    str('Sorry, you need to install the %s action for this action list.')
WARNING_LOST_VALUES = \
    str('Sorry, the values of these options will be lost in %(name)s %(version)s:')
CLIPBOARD_ACTIONLIST = \
    str('The droplet command for this action list was copied to the clipboard.')
CLIPBOARD_RECENT = \
    str('The droplet command for recent action lists was copied to the clipboard.')
CLIPBOARD_IMAGE_INSPECTOR = \
    str('The droplet command for the image inspector was copied to the clipboard.')
NO_PHOTOS = \
    str('In Phatch you need to open or create an action list first.') + ' ' + \
    str('As an example try out the polaroid action list from the library.') + \
    '\n' + \
    str('Afterwards you can drag&drop images on the Phatch window to batch them.') + \
    '\n\n' + \
    str('For more information see the tutorials (Help>Documentation)')
...
class DialogsMixin:
    ...
    def show_files_message(self, result, message, title, files):
        ...
        x0, y0 = self.GetSize()  # TODO: From Frame?
        x1, y1 = dlg.GetSize()
        x = max(x0, x1)
        y = max(y1, 200)
        dlg.SetSize((x, y))
    ...
    def show_message(self, message, title='',
        ...
        if self.IsShown():  # TODO: From Frame?
            parent = self
        else:
            parent = None
    ...
    def show_report(self):
        ...
        if report:
            ...
            self.show_message(str('No images have been processed to report.'))
    ...
    def show_log(self):
        if os.path.exists(ct.USER_LOG_PATH):
            ...
            log_file.close()
            if not msg:
                msg = str('Hooray, no issues!')
        else:
            msg = str('Nothing has been logged yet.')
        self.show_scrolled_message(msg, '%s - %s' \
                                   % (str('Log'), ct.USER_LOG_PATH))
    ...
    def show_progress_error(self, result, message, ignore=True):
        ...
        if answer == wx.ID_ABORT:
            result['answer'] = str('abort')
            self.show_log()
        elif answer == wx.ID_FORWARD:
            result['answer'] = str('skip')
        else:
            result['answer'] = str('ignore')
    ...
    def show_notification(self, message, force=False, report=None):
        ...
        active = wx.GetApp().IsActive() or self.IsActive()  # TODO: From Frame?
        if force or not active:
            notify.send(
                title=system.filename_to_title(self.filename),  # TODO: self.filename From ??
                message=message,
                icon=self.get_icon_filename(),
                wxicon=graphics.bitmap(images.ICON_PHATCH_64))
        if not active:
            self.RequestUserAttention()  # TODO: From Frame?
    ...
    def load_actionlist_data(self, filename):
        ...
        if data['invalid labels']:
            self.show_message('\n'.join([
                str('This action list was made by a different %(name)s version.') \
                % ct.INFO + '\n\n' + \
...
class Frame(DialogsMixin, dialogs.BrowseMixin, droplet.Mixin, paint.Mixin, frame.Frame, FrameReceiver):
    DEFAULT_PAINT_MESSAGE = str("click '+' to add actions")
    ...
    def _menu(self):
        ...
        setting = self.get_setting('file_history')
        _history = setting if setting else []
        self._set_file_history(_history)
        ...
        ##wx2.6 compatible
        # wx_ID_EDIT = 5030
        # self.menu_file.InsertMenu(3, wx_ID_EDIT, str("Open &Library"),
        #   library, "")
        ...
        if wx.Platform == "__WXMAC__":
            ...
            app = wx.GetApp()
            # app.SetMacHelpMenuTitleName(str('&Help'))
    ...
    def _toolBar(self):
        ...
        tools_item_other = [
            self.add_tool('ART_FILE_OPEN', str("Open"),
                          str("Open an action list"), self.on_menu_file_open)]
        tools_item = [
            self.add_tool('ART_EXECUTABLE_FILE', str("Execute"),
                          str("Execute the action"), self.on_menu_tools_execute)]
        self.frame_toolbar.AddSeparator()
        tools_item_other.extend([
            self.add_tool('ART_ADD_BOOKMARK', str("Add"),
                          str("Add an action"), self.on_menu_edit_add)])
        tools_item.extend([
            self.add_tool('ART_DEL_BOOKMARK', str("Remove"),
                          str("Remove the selected action"), self.on_menu_edit_remove),
            self.add_tool('ART_GO_UP', str("Up"),
                          str("Move the selected action up"), self.on_menu_edit_up),
            self.add_tool('ART_GO_DOWN', str("Down"),
                          str("Move the selected action down"), self.on_menu_edit_down)])
        self.frame_toolbar.AddSeparator()
        tools_item_other.extend([
            self.add_tool('ART_FIND', str("Image Inspector"),
                          str("Look up exif and iptc tags"),
                          self.on_menu_tools_image_inspector)])
        self.frame_toolbar.AddSeparator()
        self.toolbar_description = self.add_tool('ART_TIP', str("Description"),
                                                 str("Show description of the action list"),
    ...
    def on_menu_file_open_library(self, event):
        ...
        if not self.dlg_library:
            self.dlg_library = imageFileBrowser.Dialog(
                ...
                title=str('Library Action Lists'),
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if dlg.GetFilterIndex() == 0 \
                ...
                if os.path.exists(path) and self.show_question('%s %s' \
                                                               % (str('This file exists already.'),
                                                                  str('Do you want to overwrite it?'))) == wx.ID_NO:
                    saved = False
            if saved:
                self._save(path)
        else:
    ...
    def on_menu_edit_add(self, event):
        ...
        if not hasattr(self, 'dialog_actions'):
            self.dialog_actions = dialogs.ActionDialog(self,
                                                       ...
                                                       title=str("%(name)s actions") % ct.INFO)
    ...
    def on_menu_tools_python_shell(self, event):
        ...
        if self.shell is None:
            ...
            self.shell = shell.Frame(self,
                                     title=str('%(name)s Shell') % ct.INFO,
                                     intro='%(name)s ' % ct.INFO,
                                     values={
                                         '%s_%s' % (title, str('application')): wx.GetApp(),
                                         '%s_%s' % (title, str('frame')): self,
                                         '%s_%s' % (title, str('actions')): self.tree.export_forms,
                                     },
    ...
    def on_menu_help_plugin(self, event):
        ...
        dlg = dialogs.WritePluginDialog(self, '\n'.join([
            str('A html tutorial will open in your internet browser.'),
            '',
            str('You only need to know PIL to write a plugin for Phatch.'),
            str('Phatch will generate the user interface automatically.'),
            str('Study the action plugins in:') + ' ' + ct.PHATCH_ACTIONS_PATH,
            '',
            str('If you want to contribute a plugin for Phatch,'),
            str('please email: ') + ct.CONTACT]))
        dlg.ShowModal()
    ...
    def _open(self, filename):
        self._new()
        if not os.path.exists(filename):
            self.show_error(str('Sorry, "%s" is not a valid path.' % filename))
            return
    ...
    def append_save_action(self, actions):
        ...
        message = ct.SAVE_ACTION_NEEDED + " " + \
                  str("Phatch will add one for you, please check its settings.")
        if only_metadata:
            message += ' \n\n%s%s' % ( \
                str('The action list only processes metadata.'),
                str('Phatch chooses the lossless "Save Tags" action.'))
    ...
    def is_save_not_ok(self):
        if self.dirty:
            answer = self.show_message(str('Save last changes to') + '\n"%s"?' \
                                       % self.filename,
    ...
    def set_safe_mode(self, state):
        if state == False:
            answer = self.show_question(
                str('Safe mode protects you from the execution of possibly harmful scripts.\nAre you sure you want to '
                    'disable it?'),
    ...
    def get_droplet_folder(self):
        folder = self.show_dir_dialog(
            ...
            message=str('Choose the folder for the droplet'))
        if folder:
    ...
    def menu_file_export_droplet(self, method, *args, **keyw):
        ...
        try:
            ...
            self.show_info(str('Phatch successfully created the droplet.'))
        except Exception as details:
            reason = exception_to_unicode(details, WX_ENCODING)
            self.show_error(str('Phatch could not create the droplet: ') \
                            + '\n\n' + reason)
...
class DropletFrame(DialogsMixin, wx.Frame, FrameReceiver):
    ...
    def __init__(self, actionlist, paths, *args, **keyw):
        ...
        if data:
            ...
            sys.exit(str('Impossible to load data from action list.'))
...
class DropletMixin:
    ...
    def _loadSettings(self, settings):
        ...
        if os.path.exists(ct.USER_SETTINGS_PATH):
            with open(ct.USER_SETTINGS_PATH, 'r') as json_file:
                try:
    ...
    def _saveSettings(self):
        _settings = self.settings.copy()  # shallow copy
        # non permanent settings
        for key in ('desktop', 'safe', 'no_save'):
            if key in _settings:
                del _settings[key]
        # save a pretty print json
        with open(ct.USER_SETTINGS_PATH, 'w') as json_file:
            json.dump(_settings, json_file, indent=2)
...
class App(DropletMixin, wx.App):
    ...
    def __init__(self, settings: dict, filename: str, *args, **keyw):
        self._loadSettings(settings)
        self.filename = filename
