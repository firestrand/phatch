"""Unit tests for phatch.actions.rename module.

Tests the Rename action (rename original image).
...
from phatch.actions import rename as rename_action


class TestRenameAction:
    """Test the Rename action class metadata."""

    def test_action_exists(self):
        """Rename action class should exist."""
        assert hasattr(rename_action, 'Action')
        assert rename_action.Action is not None

    def test_action_has_required_metadata(self):
        """Action should have required metadata attributes."""
        action = rename_action.Action()
    ...
    def test_action_label(self):
        ...
        action = rename_action.Action()
        assert 'rename' in action.label.lower()
    ...
    def test_action_valid_last(self):
        """Rename action should be valid as last action."""
        action = rename_action.Action()
    ...
    def test_action_pil_is_none(self):
        ...
        assert rename_action.Action.pil is None


class TestRenameInterface:
    ...
    def test_interface_has_two_fields(self):
        """Rename should have two parameters (filename and folder)."""
        action = rename_action.Action()
        fields = {}
        action.interface(fields)
...
class TestIsDoneInfo:
    ...
    @patch.object(rename_action.Action, 'get_field')
...
class TestIsOverwriteForced:
    ...
    @patch.object(rename_action.Action, 'get_field_string')
    def test_is_overwrite_forced_same_location(self, mock_get_field_string):
        """Should force overwrite when renaming to same location."""
        action = rename_action.Action()
    ...
    def test_is_overwrite_not_forced_different_folder(self,
        ...
        """Should not force overwrite when renaming to different folder."""
        action = rename_action.Action()
...
class TestRenameApply:
    ...
    @patch('phatch.actions.rename.os.path.exists')
    @patch.object(rename_action.Action, 'ensure_path_or_desktop')
    @patch.object(rename_action.Action, 'is_done_info')
    def test_apply_renames_file_when_not_exists(self, mock_is_done_info,
                                                mock_ensure_path,
                                                mock_exists, mock_rename):
        """apply should rename file when destination doesn't exist."""
        action = rename_action.Action()
        ...
        mock_ensure_path.return_value = '/dest/folder/newfile.jpg'

        # Create mock photo and setting
        photo = Mock()
        photo.info = {'path': '/source/oldfile.jpg', 'type': 'jpg'}
    ...
    def test_apply_skips_when_exists_no_overwrite(self, mock_is_done_info,
                                                  mock_exists, mock_rename):
        """apply should skip rename when file exists and overwrite disabled."""
        action = rename_action.Action()
    ...
    @patch.object(rename_action.Action, 'ensure_path_or_desktop')
    @patch.object(rename_action.Action, 'is_done_info')
    def test_apply_renames_when_exists_with_overwrite(self, mock_is_done_info,
                                                      mock_ensure_path,
                                                      mock_exists, mock_rename):
        """apply should rename when file exists but overwrite enabled."""
        action = rename_action.Action()
...
class TestRenameIntegration:
    ...
    def test_action_docstring_mentions_rename(self):
        """Action documentation mentions rename."""
        action = rename_action.Action()
        doc_lower = action.__doc__.lower()
        assert 'rename' in doc_lower
