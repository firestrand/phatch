...
class TestPathConstants:
    ...
    def test_explicit_initialization_sets_user_paths(self, tmp_path: Path):
        user_paths = resolve_user_paths(
            PathResolution({}, HostPlatform.LINUX, tmp_path / "portable")
        )

        config.add_user_paths({}, user_paths)

        assert config.USER_CACHE_PATH == str(user_paths.cache)
        assert config.USER_CONFIG_PATH == str(user_paths.config)
        assert config.USER_DATA_PATH == str(user_paths.data)
...
class TestLocaleDetection:
    ...
    def test_load_locale_falls_back_to_en(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            config.locale, "setlocale", Mock(side_effect=locale.Error("fail"))
        )
    ...
    def test_load_locale_treats_empty_canonical_as_english(self, monkeypatch):
        translation = Mock()
        translate = Mock(return_value=translation)
        monkeypatch.setattr(config.gettext, "translation", translate)

        config.load_locale("phatch", "/locale", canonical="")

        assert translate.call_args.kwargs["languages"][0] == "en"
...
class TestVerifyAppUserPaths:
    ...
    def test_verify_creates_paths(self, tmp_path):
        """verify_app_user_paths should create all required directories."""
        resources = tmp_path / "resources"
        resources.mkdir()
        (resources / "geek.txt").write_bytes(b"command\n")
        user_paths = resolve_user_paths(
            PathResolution({}, HostPlatform.LINUX, tmp_path / "portable")
        )
        config._set_user_paths(user_paths)
        config.PHATCH_DATA_PATH = str(resources)

        config.verify_app_user_paths()

        assert user_paths.config.is_dir()
        assert user_paths.data.is_dir()
        assert user_paths.cache.is_dir()
        assert user_paths.geek.read_bytes() == b"command\n"

    def test_verify_requires_initialized_user_paths(self, monkeypatch):
        monkeypatch.setattr(config, "USER_PATHS", None)

        with pytest.raises(RuntimeError, match="have not been initialized"):
            config.verify_app_user_paths()

    def test_verify_skips_copy_if_geek_exists(self, tmp_path, monkeypatch):
        user_paths = resolve_user_paths(
            PathResolution({}, HostPlatform.LINUX, tmp_path / "portable")
        )
        config.initialize_user_paths(user_paths)
        user_paths.geek.write_bytes(b"existing")
        monkeypatch.setattr(config, "USER_PATHS", user_paths)
        monkeypatch.setattr(config, "PHATCH_DATA_PATH", str(tmp_path / "resources"))
        write = Mock()
        monkeypatch.setattr(config, "atomic_write_bytes", write)
...
class TestLoadLocale:
    ...
    def test_load_locale_with_default_canonical(
        self, mock_glob, mock_gettext, mock_locale
    ):
    ...
    @patch("phatch.core.config.gettext")
    @patch("phatch.core.config.glob.glob")
    def test_load_locale_with_custom_canonical(
        self, mock_glob, mock_gettext, mock_locale
    ):
    ...
    @patch("phatch.core.config.gettext")
    @patch("phatch.core.config.glob.glob")
    def test_load_locale_handles_none_canonical(
        self, mock_glob, mock_gettext, mock_setlocale
    ):
...
class TestInitConfigPaths:
    ...
    def test_init_config_paths_with_custom_paths(
        ...
        assert any(
            "/custom/python" in str(call) for call in mock_fix_path.call_args_list
        )
...
class TestLoadLocaleOnly:
    ...
    def test_load_locale_only_calls_required_functions(
        ...
        mock_check_config.return_value = {"PHATCH_LOCALE_PATH": "/test/locale"}
        ...
        mock_load_locale.assert_called_once_with("phatch", "/test/locale")
    ...
    def test_load_locale_only_with_custom_paths(
        ...
        mock_load_locale.assert_called_once_with("phatch", "/custom/locale")
